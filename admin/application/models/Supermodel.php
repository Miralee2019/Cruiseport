...
public function all_image_list($start,$page_count)
{
    $query=$this->db->query("SELECT image_library.*,T_Categorys.name FROM `image_library` INNER JOIN T_Categorys on image_library.category_id=T_Categorys.category_id order by image_library.id DESC limit $start,$page_count");
    return $query->result();
}
public function payment_detail_by_store()
{
   /* if(!empty($limit))
       { 
    $this->db->limit($limit);
}*/
$query=$this->db->order_by('id',DESC)->get('payment_details');
return $query->result();
}
public function all_sliders()
{
  $query=$this->db->order_by('id',DESC)->get('app_slider');
return $query->result();  
}
}
