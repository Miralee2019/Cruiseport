...
class Adminmodel extends CI_Model {
    ...
    function totalWalkins($store_id) {
     ...
     function totalWalkins1($store_id) {
        $q = $this->db->query('SELECT * FROM `T_BeaconActivity` WHERE T_BeaconActivity.store_id = '.$store_id.' GROUP BY user_id');
        return $q->result();
    }
...
function getTotalOfferActivity1($id) {
    $date=date('Y-m-d');
    $q = $this->db->query(
        "SELECT * FROM T_ActivityLog ta JOIN T_StoreOffer ts ON ta.store_id = ts.store_id 
        WHERE ts.offer_status = 1 AND ta.store_offer_id = ts.store_offer_id AND ts.store_id = $id and  DATE(ta.created_date) = '".$date."' AND `activity_type` != 'walkin'"
    );
        // print_r($this->db->last_query());
        // die();
    return $q->result();
...
public function all_image_list($category_name)
{
    $query=$this->db->query("SELECT image_library.*,T_Categorys.name FROM `image_library` INNER JOIN T_Categorys on image_library.category_id=T_Categorys.category_id  where T_Categorys.name='".$category_name."' order by image_library.id DESC");
    return $query->result();
}
public function payment_detail_by_store($store_id=0,$limit=0)
{
    if(!empty($limit))
       { 
    $this->db->limit($limit);
}
$query=$this->db->where('store_id',$store_id)->order_by('id',DESC)->get('payment_details');
return $query->result();
}
public function allOfferCount($store_id)
{
    $query=$this->db->query("SELECT sum(maximum_point) as total FROM `T_StoreOffer` where store_id=$store_id and offer_status=1 ORDER BY `offer_status`  DESC");
    return $query->row();
}
}
