...
<body>
    ...
    <div class="page-container">
        ...
        <div class="page-content">
                            ...
                            <div class="col-md-12" style="padding-left: 0px;padding-bottom: 20px;">
                                <div class="col-md-7" style="padding: 0;">
                                    ...
                                    <div class="col-md-5" style="padding-top: 5px;">
                                       ...
                                       <h5 style="font-size: 15px;"> First Walkin Date :
                                        <?php foreach ($first_w_date as $key) { ?>
                                        <?php if ($key === reset($first_w_date)) { ?>
                                        <?php echo $key->detected_date; ?>
                                        <?php } else { echo 'NA'; } } ?></h5>
                                      
                                     <h5 style="font-size: 15px;"> Last Walkin Date : 
                                        <?php
                                        foreach ($last_w_date as $key) {
                                            ?>
                                            <?php if ($key === end($last_w_date)) { ?>
                                            <?php echo $key->detected_date; ?>
                                            <?php
                                        } else { echo 'NA'; }
                                    }
                    ...
                    <div class="panel-body padding10">
                        ...
                        <div class="col-md-9 col-xs-6">
                           ...
                           /*     if (isset($SumPOint)) {
                                    ...
                                    $point = $this->Adminmodel->select_data('T_SocialSharePoint', array('store_id' => $this->session->userdata('store_id'),'user_id'=>$key->user_id));
                                                    $point1 = $this->Adminmodel->select_data('T_Userstorewalkinpoint', array('store_id' => $this->session->userdata('store_id'),'user_id'=>$key->user_id));
                                                 //   print_r($this->db->last_query());die;
            if(!empty($point))
            {
                $social=0;
               foreach ($point as $key ) {
                if(!empty($key->facebook_points))
                  $social=$social + $key->facebook_points;   
               }
               if(!empty($key->twitter_points))
               {
                  $social=$social + $key->twitter_points;   
               }
            }else{
                $social=0;
            }
            ...
            {
                $walkin=0;
               foreach ($point1 as $key ) {
                if(!empty($key->walkin_points))
                  $walkin=$walkin + $key->walkin_points;   
               }
                            ...
                            <tr>
                                                     ...
                                                     if ($key->activity_type == "share") {
                                                        ...
                                                        $query=$this->db->where('store_offer_id',$store_offer_id)->get('T_StoreOffer');
                                                                ...
                                                                if ($point[$m]->share_type == "facebook_points") {
                                                                    ...
                                                                    echo $key->activity_name." <b>".$result->title."</b>";
                                                                } else {
                                                                    echo $key->activity_name." <b>".$result->title."</b>";
                                                                }
                                                            }else if($key->activity_type == "walkin points")
                                                                {
                                                                     $store_id=$key->store_id;
                                                                      $query=$this->db->where('store_id',$store_id)->get('T_Store');
                                                                  $result=$query->row();
                                                                     echo "<b>$result->walking_point</b> ".$key->activity_name;
                                                                }else if($key->activity_type == "store_send")
                                                                {
                                                                     echo $key->activity_name;
                                                                }else if($key->activity_type == "favorite_offer")
                                                                {
                                                                      $store_id=$key->store_id;
                                                                    $store_offer_id=$key->store_offer_id;
                                                        $query=$this->db->where('store_offer_id',$store_offer_id)->get('T_StoreOffer');
                                                                  $result=$query->row();
                                                                     echo $key->activity_name." name <b>".$result->title."</b>";
                                                                }
                                                                 else {
                                                                echo $key->activity_name;
                                                            }
                                    ...
                                    <span style="background: #01a8f6; padding: 4px 0px 2px 0px; border-radius: 4px; color: #fff;">
                                        <?php
                                                            if ($key->activity_type == "share") {
                                                                ...
                                                                {
                                                                     echo "Store Notification";
                                                                }else if($key->activity_type == "favorite_offer")
                                                                {
                                                                     echo "Favorite Offer";
                                                                }else if($key->activity_type == "favorite_store")
                                                                {
                                                                     echo "Favorite Store";
                                                                }
                                                                 else {
                                ...
                                <td class="col-md-2">
                                    ...
                                    if($key->activity_type == "share") {
                                       ...
                                       $query= $this->db->query("select walkin_points from T_Userstorewalkinpoint where DATE(`created_date`)='".rtrim($key->activity_date,'-')."'");
                                       $result=$query->row();
                                     //  print_r($this->db->last_query());
                                       if(!empty($result))
                                       {
                                        if($key->activity_type=="walkin points" || $key->activity_type=="Facebook Share" || $key->activity_type=="Twitter Share"){
                                       echo $result->walkin_points;
                                   }else{
                                    echo "-";
                                   }
                                        }else{ 
