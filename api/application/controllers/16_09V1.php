    ...
    function verifyPhoneOtpAuth_post() {
        ...
        if (!$key) {
            ...
            if ($res1 > 0) {
               ...
               if(!empty($otptime)){ if ($otptime[0]->otptime < 120) { 
    ...
    function allStoreDetails_post() {
        ...
        if (!$key) {
         ...
         //   $geocode = array();
            ...
            $addr = empty($output->results[0]->formatted_address) ? 'Bavdhan, Pune, Maharashtra 411021, India': $output->results[0]->formatted_address; 
        ...
        if (!$key) {
            ...
            if ($coupon_data > 0) {
                ...
                foreach ($coupon_data as $key) {
                    ...
                    $coupon_visi = $this->Apimodel->select_data('T_CouponClaims', array('user_id' => $post["user_id"],
...
$final_array=[];
  ...
  $is_already_rated_store_by_user = $this->Apimodel->select_data('T_UserReviews', array('store_id' => $store_id, 'user_id' => $post['user_id']));
                    ...
                    $store_balance = $this->Apimodel->select_data('T_Store', array('store_id' => $offer_data->store_id));
                        ...
                        $offers=[   "store_offer_id"=> $offer_data->store_offer_id,
