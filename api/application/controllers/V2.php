...
define("upload_url", "http://cruiseportshopping.com/cashrub/admin/");
        ...
        if (($post["is_social"]) == "1") {
            if ($post["fb_userid"] != '') {
                ...
                if ($after_delete || $chk_store_email || $email) {
					...
					$portdata = $this->Apimodel->get_port($insert_id[0]->user_id);
    ...
    function userLogin_post()
    {
        ...
        if (($post["is_social"]) == "1") {
                ...
                if ($res) {
                    ...
                    if ($res[0]->otp_verify == 1) {
                        ...
                        if ($res[0]->status_id == 1) {
							...
							$portdata = $this->Apimodel->get_port($data[0]->user_id);
                            $result = [
            ...
            if (!$key) {
                ...
                $res = $this->Apimodel->select_data('T_User', array('phone_number' => $post['phone_number']), 1);
                ...
                if ($res > 0) {
                    // if (empty($result)) {
                        ...
                        if ($res1 > 0) {
							...
							$portdata = $this->Apimodel->get_port($res1[0]->user_id);
                            $result = [
                            ...
                            //chintan :: update country code
							if ($res[0]->country_code == "") {	                         
                                $update_device_token = $this->Apimodel->update('T_User',array('user_id' => $res[0]->user_id), array('country_code' => $post["country_code"]));
								}
                            ...
                            */
                                ...
                                $update_device_token = $this->Apimodel->update('T_User', array('phone_number' => $post["phone_number"]), array('device_token' => $post['device_token'], 'device_id' => $post['device_id'], 'access_token' => $accesstoken)); // Subodh :: Uncommented so that device token should update on login as per Android 
                                ...
                                // } else {
                                    ...
                                    try {
                                        ...
                                        $str = $str;
                                        $match = $str;
										$otp_data['country_code'] = $post['country_code'];
                                        ...
                                        if ($auth_otp) {
                                            $postotp = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+" . $post['country_code']. trim($post['phone_number']) . "&message=%3C%23%3E+Your+OTP+is " . $match . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
            ...
            if ($res1 > 0) {
                ...
                if ($res > 0) {
                    ...
                    if ($res[0]->phone_number == '') {
                        ...
                        $post = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+" .$post['country_code']. $post['phone_number'] . "&message=%3C%23%3E+Your+OTP+is " . $match2 . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
        ...
        if (!$key) {
            ...
            if ($res > 0) {
                ...
                if ($res[0]->phone_number == '') {
                    ...
                    // $phone = $this->Apimodel->select_data('T_User', array('user_id' => $post["user_id"]));
                    $phone = $res[0]->country_code.$res[0]->phone_number;
                    // var_dump($phone[0]->phone_number);die;
                    for ($i = 5; $i > 0; $i--) {
                        $str2 = rand(1000, 9999);
                    }
                    ...
                    $post = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+". phone . "&message=%3C%23%3E+Your+OTP+is " . $match2 . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
                    // var_dump($match2);die;
                    // var_dump($updateOtp);die;
                    //  $again = $this->Apimodel->update('T_User', array('user_id'=>$post['user_id']), array('otp' => '1111') );
    ...
    */
        ...
        if (!$key) {
            ...
            if ($res > 0) {
                ...
                if ($res[0]->phone_number == '') {
                    ...
                    $post = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+" .$cc . $phone . "&message=%3C%23%3E+Your+OTP+is " . $match2 . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
                    //    print_r($post);die;
                    // var_dump($updateOtp);die;
                    //  $again = $this->Apimodel->update('T_User', array('user_id'=>$post['user_id']), array('otp' => '1111') );
    ...
    function getProfile_post()
    {
        ...
        if (!$key) {
            ...
            if ($profile) {
				$portdata = $this->Apimodel->get_port($profile[0]->user_id);
                $result = [
        ...
        if (!$key) {
            ...
            if ($user_phone_chk) {
                ...
                $post_arr2 = [
					...
					'country_code' => $user_database[0]->country_code ?: '',
        ...
        if (!$key) {
            ...
            if ($post['rubs'] < $t_p_o_u) {
                ...
                if (!empty($user_chk) && !empty($prev_claims)) {
                  ...
                  Should have any query? Reach us to at contact@cruiseportshopping.com <br>
                  <br><br>
                   Thank You.';
            ...
            if ($coupon_data > 0) {
                ...
                foreach ($coupon_data as $key) {
                    ...
                    $result[] = array(
                        ...
                        "coupon_title" => $key->coupon_title ?: '',
                        "description" => $key->coupon_description ?: '',
                        "coupon_image" => 'http://cruiseportshopping.com/cashrub/admin/uploads/' . $key->coupon_image ?: '',
                        "coupon_expiry" => $key->coupon_expiry_date ?: '',
                        "provider" => $key->store_id ?: '',
                        "category_name" => $key->name ?: '',
                        "claims" => $key->claims,
                ...
                if ($user_offer_favorites) {
                    foreach ($user_offer_favorites as $key) {
                        ...
                        $offer[] = array(
                            ...
                            "store_logo" => 'http://cruiseportshopping.com/cashrub/admin/uploads/' . $store[0]->store_logo ?: '',
                            ...
                            "offer_image" => 'http://cruiseportshopping.com/cashrub/admin/uploads/' . $offer_data[0]->offer_image,
...
Should have any query? Reach us to at contact@cruiseportshopping.com <br>
    ...
    function getDemoLocation_get()
    {
        ...
        $agent = 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV2; .NET CLR 1.0.3705; .NET CLR 1.1.4322)';
    ...
    */
    ...
    function otp_auth_post()
    {
        ...
        if (!$key) {
            ...
            try {
                ...
                if ($auth_otp) {
                    $post = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+" . $post['country_code'].trim($post['phone_number']) . "&message=%3C%23%3E+Your+OTP+is " . $match . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
        ...
        if (!$key) {
            ...
            if (!empty($user_details)) {
                try {
                    ...
                    //$auth_otp = TRUE; // SUBODH :: This is for testing
                    if ($auth_otp) {
                        $post = file_get_contents("https://control.msg91.com/api/sendhttp.php?authkey=151244A0M4jedLtf590b0ff2&mobiles=+" . trim($post['country_code'].$post['phone_number']) . "&message=%3C%23%3E+Your+OTP+is " . $match . "+" . $this->google_sms_key . "&sender=CASHRUB&route=4&country=0");
                    ...
                    if (!empty($offer_data)) {
                        ...
                        foreach ($offer_data as $offer_data) {
                            ...
                            $offers = [
                                ...
                                "offer_image" => 'http://cruiseportshopping.com/cashrub/admin/uploads/' . $offer_data->offer_image,
                      ...
                      //  if ($km <= $distance) {
                            $final[] = [
                                ...
                                'store_logo' => 'http://cruiseportshopping.com/cashrub/admin/uploads/' . $store['store_logo'],
        ...
        if (!$key) {
            ...
            foreach ($result as $res) {
                ...
                $data = [
                    ...
                    'slider_image' => 'http://cruiseportshopping.com/cashrub/admin/slides/' . $res->slider_image,
                    'type' => $res->type,
	...
	// get ship dat and port
	
	  function getship_post()
    {
        $post = json_decode(file_get_contents("php://input"), true);
      //  $keys = array_keys($post); //convert json into array formate
      //  sort($keys); //sort array to campare with sample string
        //$sample = '["user_id","access_token"]';
        $ship = $this->Apimodel->select_data('ship', array('1 =' => "1"));
        if ($ship) {
            foreach ($ship as $key) {
                $result[] = array(
                    "ship_id" => $key->ship_id ?: '',
                    "ship_name" => $key->ship_name ?: ''
                );
            }

            $responsearray = array("status" => 2000, "success" => true, "message" => "ship list.", "result" => $result);
            $this->response($responsearray, REST_Controller::HTTP_OK);
        } else {

            $responsearray = array("status" => 6013, "success" => false, "message" => "ship not found.", "result" => new stdClass());
            $this->response($responsearray, REST_Controller::HTTP_OK);
        }
    }
	
	  function getport_post()
    {
        $post = json_decode(file_get_contents("php://input"), true);
        $keys = array_keys($post); //convert json into array formate
        sort($keys); //sort array to campare with sample string
        ...
        $port = $this->Apimodel->select_data('port', array('ref_ship_id' => $required['ship_id']));
        ...
        if ($port) {
            ...
            $responsearray = array("status" => 2000, "success" => true, "message" => "Port list.", "result" => $result);
            $this->response($responsearray, REST_Controller::HTTP_OK);
        } else {

            $responsearray = array("status" => 6013, "success" => false, "message" => "Port not found.", "result" => new stdClass());
            $this->response($responsearray, REST_Controller::HTTP_OK);
        }
		} else {
			$key = json_encode(array_values($key));
	...
	// update only port and get user data
	 function updatePort_post()
    {
        ...
        if (!$key) {
            ...
            $post_arr = [
                'ref_port_id' => $post["port_id"]
            ];

            $res = $this->Apimodel->update('T_User', array('user_id' => $post["user_id"]), $post_arr);
			$points = $this->Apimodel->select_data('T_UserPoint', array('user_id' => $post["user_id"]), 1);
			if (!empty($points)) {
				$total_points = ($points[0]->facebook_points) + ($points[0]->twitter_points) + ($points[0]->walking_points) + ($points[0]->reward_points) - ($points[0]->coupon_redeem_rubs);
			} else {
				$total_points = 0;
            ...
            $res1 = $this->Apimodel->select_data('T_User', array('user_id' => $post["user_id"]));
			//print_r($post["port_id"]);die;
			$portdata = $this->Apimodel->get_port($post["user_id"]);
        ...
        /*    $post_arr2 = [
                ...
                'gender' => $user_de_get[0]->gender ?: '',
                'profile_image' => $user_de_get[0]->profile_image ? base_url() . "uploads/" . $user_de_get[0]->profile_image : '',
				'ref_port_id' => $user_de_get[0]->ref_port_id ?: '',
				'portdata' => $portdata
            ];*/
			 ...
			 $result = [
                                ...
                                "gender" => $res1[0]->gender ?: '',
                                "device_token" => $res1[0]->device_token ?: '',
                                "device_id" => $res1[0]->device_id,
                                "access_token" => $res1[0]->access_token,
                                // "total_points"=> ($points[0]->facebook_points)+($points[0]->twitter_points)+($points[0]->walking_points),
                                "total_points" => $total_points,
                                "profile_image" => $res1[0]->profile_image ? base_url() . "uploads/" . $res1[0]->profile_image : '',
                                "is_email" => $res1[0]->email_verified,
            ...
            $responsearray = array("status" => 2000, "success" => true, "message" => "User profile updated successfully", "result" => $result);
            $this->response($responsearray, REST_Controller::HTTP_OK);
        } else {
